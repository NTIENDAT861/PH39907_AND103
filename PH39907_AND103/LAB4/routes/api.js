...
//send maill
router.post("/register-send-mail", Upload.single("avatar"), async (req, res) => {
  await mongoose.connect(uri);
  try {
    const data = req.body;
    const { file } = req;
    ...
    if (result) {
      //gửi mail
      const mailOptions = {
        from: "ntiendat.nofriendnomoney@gmail.com",
        to: result.email,
    ...
    // Handling the error should include sending an appropriate response to the client
